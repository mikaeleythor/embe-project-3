...
double PI_Controller::update(double ref, double actual) {
    ...
    integral += error * (getUpdateRateMs() / 1000.0); // Scale by time step

    double control_signal = getKp() * error + Ki * integral;
...
double PI_Controller::applySaturation(double control_signal, double error) {
    if (control_signal > 255) {
        control_signal = 255;
        integral -= error * (getUpdateRateMs() / 1000.0); // Prevent windup
